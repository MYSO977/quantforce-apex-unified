    ...
    def fetch_node(n):
        try:
            if n["local"]:
                ...
                ssh.connect(n["ip"], username="heng", timeout=1,
            ...
            return {**n, "alive": True, "cpu_pct": round(cpu_used,1), "mem_pct": round(mem_pct,1), "mem_mb": mem_mb, "procs": procs}
        except Exception as e:
            return {**n, "alive": False, "cpu_pct": 0, "mem_pct": 0, "mem_mb": 0, "procs": []}

    results = [None] * len(nodes)
    with ThreadPoolExecutor(max_workers=5) as ex:
        futures = {ex.submit(fetch_node, n): i for i, n in enumerate(nodes)}
